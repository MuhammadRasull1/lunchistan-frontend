...
- Кнопка «Оформить предзаказ» в **StickyBar**
...
- **Способ оплаты**: 3 варианта (см. п. 4)
- **Итоговая сумма**: жирный оранжевый текст
- **Кнопка оплаты**: `btn--primary btn--lg` на всю ширину

### 3.2. Детализация строки сета

```
🍱 Обед День 1 (Пн)
    День 1 · Пн · Вода · 2 порц./сотр.
                             2 × 55 000 сум
                             110 000 сум
```
...
| Метод          | Значение      | Описание                 | Эмодзи | По умолчанию |
| -------------- | ------------- | ------------------------ | ------ | ------------ |
| Перечислением  | `'corporate'` | Для юридических лиц      | 🏢     | ✅ Да        |
...
2. **Заголовок**: «Заказ оформлен!»
3. **Текст**: «Спасибо! Мы уже передали заказ на кухню Lunchistan.»
4. **Номер заказа**: `#ORD-NNNN` (случайный 4-значный, генерируется при монтировании)
5. **Кнопка**: «Сделать новый заказ» → полный сброс ([[STATE_MANAGEMENT#7-сброс-состояния-new-order]])
