...
- **Выбор напитка** — стильные pill-кнопки (`beverage-pill`) под составом: 💧 Вода / 🍷 Компот. Выбор сохраняется мгновенно в [[STATE_MANAGEMENT]] через `onBeverageChange`
...
- ❌ **Удалено**: вложенная карточка с градиентным фоном и круглой иконкой
...
interface SetDetailModalProps {
  ...
  beverage: Beverage      // текущий выбранный напиток
  onBeverageChange: (beverage: Beverage) => void  // смена напитка
}
```
