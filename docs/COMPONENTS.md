...
4. 🆆 **v3.2 «Выбранные дни»** — список токенов: строка на каждую выбранную дату (`.selected-days .day-row`), клиент **сам выбирает блюдо** на день, если блюд на дату несколько. Заголовок секции — бейдж `.tokens-badge` («Свободных токенов: N» / «Блюда выбраны на все дни ✓», `freeTokens = activeDays − chosenDaysCount`) + подсказка `tokensHint`. Клик по названию выбранного блюда → `SetDetailModal` (кастомизация салата/напитка/порций). День без блюда — `.day-row--empty` + «Токен свободен» (или «Загрузка…», пока `dayMenus[date]` не пришёл). Показывается только при `activeDays > 0`.
   - 🆆 **v3.4 (17.09.2026):** источник блюд дня — `dayMenus[date]` (дневное меню этой даты, не весь каталог из 56 позиций). Кнопка «Выбрать блюдо» / «Изменить блюдо» показывается **только если на дату предложено больше одного блюда** (`dayMenus[date].length > 1`) → открывает `SetPicker` со списком именно этой даты. Если блюдо ровно одно — оно назначается автоматически (`App.tsx`, `ensureDayMenu`), кнопки нет, менять нечего.
5. **Полное меню на месяц** — табы категорий + сетка SetCard × **все** сеты (всегда). 🆆 v3.2: карточка «активна», если это блюдо выбрано хотя бы на один день (`chosenSetIds`); клик = read-only предпросмотр (управление днями — в секции «Выбранные дни»). ⚠️ **Открытый вопрос (17.09.2026):** с переходом на ежедневное меню этот раздел показывает **весь активный каталог**, а не то, что реально доступно на выбранные клиентом даты — по сути стал общим read-only обзором ассортимента, а не «меню на месяц». Оставлен как есть (не удалялся по прямому указанию задачи); имеет смысл пересмотреть заголовок/назначение раздела отдельной задачей.
6. **StickyBar** — фиксированная панель с итогом + кнопка «Оформить». 🆆 v3.2: кнопка (и Telegram `MainButton`) заблокирована с подсказкой `chooseDishForEveryDay`, пока не выбрано блюдо на **каждый** день (`allDishesChosen`).
7. **SetDetailModal** — выплывающее окно детализации сета (кастомизация дня / read-only предпросмотр)
8. 🆆 **SetPicker** — bottom-sheet со всеми 56 сетами и поиском (тот же, что в `EmployeeView`); `onPick(setId)` → `onSetChange(date, setId)`
...
interface CatalogProps {
  ...
  menu: LunchSet[]                    // полный активный каталог (read-only обзор, резолв id, опции салата)
  dayMenus: Record<string, LunchSet[]> // 🆆 v3.4 дневное меню по дате (undefined — не загружено, [] — не внесено)
  allSetsCount: number
  ...
  allDishesChosen: boolean                        // 🆆 v3.2 все дни имеют выбранное блюдо
  onSetChange: (date: string, setId: number) => void  // 🆆 v3.2 клиент выбрал блюдо на день
  onGoToCart: () => void
...
- 🆆 **v3.2:** блюдо на день выбирает **сам клиент** (модель токенов, как в [[COMPONENTS#123-employeeviewtsx--мои-дни-роль-employee]]), если на дату предложено больше одного блюда. Карточка сета в каталоге «активна» = это блюдо выбрано хотя бы на один день.
- 🆆 **v3.4 (17.09.2026):** блюда бизнеса не повторяются день в день — ротация `getSetForDate` удалена. Блюдо на дату приходит из `dayMenus[date]` (`fetchDayMenu`, кеш в `App.tsx`); если оно одно — назначается автоматически без похода в `SetPicker`.
...
- 🆆 **v2.3 (17.09.2026):** дополнительно недоступны даты **без внесённого меню** — при открытии модалка запрашивает `fetchAvailableDates(from, to)` (окно текущий→следующий месяц) и комбинирует результат с `canSelectDate` в `canPick(date) = canSelectDate(date) && hasMenu(date)`. Пока список не загружен, новые даты считаются недоступными (fail-closed); уже выбранные ранее даты остаются кликабельными (можно снять), даже если их меню задним числом пропало. Локер получает тултип `dateMenuNotReady` («Меню на эту дату ещё не готово»); ошибка загрузки списка — вторая строка хинта (`menuLoadError`).
- 🆆 **v2.4 (23.09.2026, «не заставляй пользователя думать»):** `title`-тултип на disabled-кнопке невидим на тач-экранах (Telegram Mini App — там нет hover), поэтому если на весь видимый месяц не внесено ни одного меню, пользователь видел молча заблокированный календарь без объяснения. Добавлен видимый баннер `.calendar-modal__hint--warning` (ключ `calendarMonthNotReady`) — показывается, когда `monthSelectableDates.length === 0` после загрузки `availableDates`. Пресеты и «Вся неделя» молча пропускают недоступные даты (`presetCandidates`).
...
### 12.5. SetPicker.tsx — выбор блюда (bottom-sheet, 🆆 v3.2 — фото-сетка)
- Список сетов **карточками с фото** (`SetCard`, `preview`), а не текстовым списком. Поиск по названию + табы категорий (`.set-picker__tabs`, лента скроллится вбок). Текущий выбор — рамка `.set-picker__card--current` + галочка. Клик по карточке → `onPick(setId)` + закрытие.
- Props: `isOpen`, `menu: LunchSet[]`, `lang`, `current: SetOfDay | null`, 🆆 `dayLabel?` (подпись дня в заголовке), `onPick`, `onClose`.
- Используется в `EmployeeView` («Команды», полный каталог `menu`) **и** в `Catalog` (клиентский поток, секция «Выбранные дни»). `Number(set.id)` — `LunchSet.id` тип `string | number`.
- 🆆 **v3.4 (17.09.2026):** в `Catalog` компонент получает `menu={dayMenus[pickForDate] ?? []}` — **дневное меню** конкретной даты, а не весь каталог (блюда бизнеса не повторяются день в день). `EmployeeView` по-прежнему передаёт полный каталог (модель «Команд» не менялась этой задачей).
