...
- Карточка сета активна, если в выбранных датах есть день с тем же числом месяца (`set.dayNumber`): показывается бейдж реальной даты, клик открывает кастомизацию этого дня. Остальные карточки — предпросмотр сета (read-only).
...
- **Все календарные даты выбираемы**, включая субботу и воскресенье (выходные стилизованы `.calendar__day--weekend`, но кликабельны).
...
- **Пресет «Вся рабочая неделя»**: кнопка `CalendarPlus` у каждой строки недели выбирает Пн–Пт именно той недели (повторное нажатие снимает).
- **Глобальные пресеты** `2/2`, `5/2`, `6/1`, `Весь месяц` — **toggle** (v2.2): первое нажатие добавляет даты пресета в видимый месяц (другие месяцы не затрагиваются), повторное — снимает **ровно пресетные даты**, а вручную добавленные/снятые дни сохраняются. `full` («Весь месяц») выбирает только Пн-Пт видимого месяца.
...
- **Быстрые действия:** «Выбрать все N дней» (все даты видимого месяца, включая выходные) / «Сбросить все» (только видимый месяц) — на черновике. Работают независимо от пресетов.
- **Нижняя плашка:** счётчик «Выбрано дней: N» + кнопка «Подтвердить».

**Визуальные состояния:**
- Выбранный день — оранжевый (`--brand`)
- Выходной невыбранный — тёплый фон (`.calendar__day--weekend`)
