...
4. 🆆 **v3.2 «Выбранные дни»** — список токенов: строка на каждую выбранную дату (`.selected-days .day-row`), клиент **сам выбирает блюдо** на день. Кнопка «Выбрать блюдо» / «Изменить» → `SetPicker`; клик по названию выбранного блюда → `SetDetailModal` (кастомизация салата/напитка/порций). День без блюда помечен `.day-row--empty` + предупреждение. Показывается только при `activeDays > 0`.
