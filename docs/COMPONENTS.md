...
4. 🆆 **v3.2 «Выбранные дни»** — список токенов: строка на каждую выбранную дату (`.selected-days .day-row`), клиент **сам выбирает блюдо** на день. Заголовок секции — бейдж `.tokens-badge` («Свободных токенов: N» / «Блюда выбраны на все дни ✓», `freeTokens = activeDays − chosenDaysCount`) + подсказка `tokensHint`. Кнопка «Выбрать блюдо» / «Изменить блюдо» (во всю ширину строки) → `SetPicker` (фото-сетка); клик по названию выбранного блюда → `SetDetailModal` (кастомизация салата/напитка/порций). День без блюда — `.day-row--empty` + «Токен свободен». Показывается только при `activeDays > 0`.
5. **Полное меню на месяц** — табы категорий + сетка SetCard × **все** сеты (всегда). 🆆 v3.2: карточка «активна», если это блюдо выбрано хотя бы на один день (`chosenSetIds`); клик = read-only предпросмотр (управление днями — в секции «Выбранные дни»).
...
- 🆆 **v3.2:** блюдо на день выбирает **сам клиент** (модель токенов, как в [[COMPONENTS#123-employeeviewtsx--мои-дни-роль-employee]]). `getSetForDate` (ротация) больше **не** назначает блюдо в клиентском потоке — остаётся лишь как визуальный плейсхолдер для ещё не выбранного дня. Карточка сета в каталоге «активна» = это блюдо выбрано хотя бы на один день.
...
- 56 сетов `MONTHLY_SETS` **карточками с фото** (`SetCard`, `preview`), а не текстовым списком. Поиск по названию + табы категорий (`.set-picker__tabs`, лента скроллится вбок). Текущий выбор — рамка `.set-picker__card--current` + галочка. Клик по карточке → `onPick(setId)` + закрытие.
- Props: `isOpen`, `lang`, `current: SetOfDay | null`, 🆆 `dayLabel?` (подпись дня в заголовке), `onPick`, `onClose`.
- Используется в `EmployeeView` («Команды») **и** в `Catalog` (клиентский поток, секция «Выбранные дни»). `Number(set.id)` — `LunchSet.id` тип `string | number`.
