...
### 3.5. CalendarModal.tsx — Модалка выбора дат (v2.1) 🆆
...
- **Глобальные пресеты** `2/2`, `5/2`, `6/1`, `Весь месяц` — перестраивают черновик **в пределах видимого месяца** (даты других месяцев не затрагиваются).
- **Быстрые действия:** «Выбрать все N дней» (все даты видимого месяца, включая выходные) / «Сбросить все» (только видимый месяц) — на черновике.
