...
> 🆆 **v3.2:** в клиентском потоке блюдо на день выбирает **сам клиент** (`cartState[date].setId`, резолв `getSetById`, [[STATE_MANAGEMENT#22-cartstate--детальная-структура]]). `getSetForDate` ниже — теперь лишь **плейсхолдер** для ещё не выбранного дня; оформить заказ нельзя без блюда на каждый день.

### 6.1. Глобальная порядковая привязка (плейсхолдер невыбранного дня)
...
- Используется в `App.tsx` (`orderDays` — fallback, если `item.setId` не выбран). Прошлые даты не приводят к выходу за границы массива (двойной модуль безопасен).
- 🆆 `getSetById(id)` — резолв блюда по выбору клиента (`item.setId`).
...
- **Временная резка «сегодня»**: `TODAY_ORDER_CUTOFF_HOUR = 10` — заказ на сегодня возможен только до 10:00. Единый предикат `canSelectDate(date)` (не прошлое И, если сегодня, то до резки) используется во всех путях выбора: клик, «Вся неделя», пресеты `2/2`/`5/2`/`6/1`/`full` (через `buildPresetDates`), «Выбрать все».
