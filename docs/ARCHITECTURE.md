...
> Версия: 3.2 — хозяйские функции (OwnerView) переехали в lunchistan-core  \
...
> заказа и контур «Команды» работают через `lunchistan-backend`
> (`VITE_API_BASE_URL`, по умолчанию `https://lunchistan-backend.onrender.com`).
> **21.09.2026:** все хозяйские функции (сводка, меню, настройки оплаты) переехали в
> отдельное приложение `lunchistan-core` — здесь `OwnerView` больше нет, роль `owner`
> в этом приложении видит только вежливое сообщение (см. [[COMPONENTS#13-раздел-кабинет]]).
