...
- Глобальные пресеты `2/2`, `5/2`, `6/1`, `full` применяются к **черновику в пределах видимого месяца** (даты других месяцев не затрагиваются). Паттерн начинается от 1-го числа видимого месяца и применяется к последовательности Пн-Пт (реализация `buildPresetDates()` в [[ARCHITECTURE#3-типы-данных-srctypests]] / `src/lib/calendar.ts`):
